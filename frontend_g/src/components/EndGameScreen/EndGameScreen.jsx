...
const EndGameScreen = ({ game, result, onPlayAgain }) => {
  const [leaderboard, setLeaderboard] = useState([]);
  const [playerName, setPlayerName] = useState("");
  const [scoreSubmitted, setScoreSubmitted] = useState(false);

  const wasWin = result.status === "won"; // Simple check if the game was won
  ...
  const handleSubmitScore = async (e) => {
    ...
    try {
      await fetch(`/api/games/${game.id}/scores`, {
        ...
        body: JSON.stringify({ playerName, timeInSeconds: result.time }),
      });
  ...
  return (
    <div className="endgame-container">
      <div className="endgame-card">
        ...
        {wasWin && (
          <p className="final-time">Your time: {formatTime(result.time)}</p>
        )}
        ...
        {!wasWin && <p className="final-time">Better luck next time!</p>}
